...
static float feedrate = 1500.0, next_feedrate, saved_feedrate;  //Try to set z-probe up-running z-axis feedrate (orginal = 1500.0)
...
void setup_solenoidpin()
{
//  #ifdef PHOTOGRAPH_PIN
//    #if (PHOTOGRAPH_PIN > -1)
    pinMode(SOLENOID_PIN, OUTPUT);
    WRITE(SOLENOID_PIN, LOW);
//    #endif
//  #endif 
}
...
static void do_blocking_move_to(float x, float y, float z) {
    ...
    feedrate = homing_feedrate[Z_AXIS]; // orginal = [X_AXIS]
    //feedrate = 300;
...
void process_commands()
{
  ...
  if(code_seen('G'))
  {
    ...
    case 28: //G28 Home all Axis one at a time
      ...
      if((home_all_axis) || (code_seen(axis_codes[Z_AXIS]))) {
        //digitalWrite(SOLENOID_PIN, HIGH); // doing lower Z-probe by Solenoid.
        HOMEAXIS(Z);
        /digitalWrite(SOLENOID_PIN, LOW); // doing upper Z-probe by Solenoid.
      }
      #endif
      ...
      if((home_all_axis) || (code_seen(axis_codes[Z_AXIS]))) {
        //digitalWrite(SOLENOID_PIN, HIGH); // doing lower Z-probe by Solenoid.
        HOMEAXIS(Z);
        //digitalWrite(SOLENOID_PIN, LOW); // doing lower Z-probe by Solenoid.
      }
      #endif
  ...
  else if(code_seen('M'))
  {
    ...
    break;
    case 401:
    //SET_OUTPUT(SOLENOID_PIN, OUTPUT);
    WRITE(SOLENOID_PIN, HIGH);
//#if defined(ENABLE_AUTO_BED_LEVELING) && defined(LOWER_AND_RAISE_Z_PROBE)
      //lower_z_probe();
//#endif
